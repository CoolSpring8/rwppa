...
> ⚠️ 本仓库已归档，不再维护。
>
> 由于本项目仅作为概念验证（PoC）设计，其安全性与完备性有所欠缺。
>
> 为了拦截 HTTPS 流量，rwppa 会在本地生成根证书，由用户手动安装。
> 尽管 MITM 风格的处理代码本身开源透明，但是当前实现下保存在本地的根证书文件存在被恶意利用的可能。
>
> 出于安全因素考虑，**请勿继续使用此方案**。如果你之前使用过 rwppa，强烈建议从系统及浏览器的受信任证书存储中移除相关证书。
>
> 可能的替代方案参见下方项目列表。未经过验证，请自行研究甄别。
